  ...
  return (
    <div className={className}>
      <div className="text-lg font-main font-bold text-white bg-gradient-to-r from-green to-blue h-14 px-8 py-2 flex gap-16 xl:w-[98.7vw] lg:h-20 xl:h-14">
            ...
            <div className="text-base flex gap-4 py-1.5 lg:gap-3 xl:gap-4">
                {NavBarLinks.map((link) => (
                    <Link to={link.to} key={link.id} className="cursor-pointer text-white px-2 hover:rounded-lg hover:bg-orange">{link.link}</Link>
                ))}
            </div>
            <div className="py-1 lg:py-4 xl:py-1">
                <SearchBar />
            </div>
            <div className="flex py-1.5 gap-8 lg:py-4 xl:py-1.5 lg:gap-3 xl:gap-8">
